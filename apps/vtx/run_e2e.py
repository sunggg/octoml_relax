...
# NOTE(areusch): Work around "TensorIntrin 'wmma_fill_16x16x16_f16' is not registered" error.
...
SRC_FILE = "./fmha.cu"
PKG_FILE = "./packaged.o" if USE_STATIC_LIB else "./packaged.so"
...
if __name__ == "__main__":
    ...
    if RUN_COMPILE:
        with target:
            ...
            if USE_STATIC_LIB:
                kw["fcompile"] = create_archive
                kw["options"] = ["-Xcompiler=-r"]

            relax_ex.export_library(PKG_FILE, **kw)
            print("Exported")
    executable = tvm.runtime.load_module(PKG_FILE)
    if not USE_STATIC_LIB:
        executable = tvm.relax.vm.Executable(executable)
    ...
    if IS_CI:
        print("Not running ONNX tests in CI because they don't work in ci_gpu")
        sys.exit(0)
